"""
Plot trial-averaged PSTH and LFP from cleo trial data (trials_cleo.py output).

Produces:
  1. Per-channel PSTH (one row per electrode channel)
  2. PSTH averaged by compartment (SG / G / IG)
"""
import os
import glob
import numpy as np
...
def build_psth_from_arrays(mua_t_ms, mua_i, n_channels, stim_onset_ms,
    ...
    """Build PSTH from saved MUA arrays (times in ms, channel indices)."""
    t_pre = t_pre_ms / 1000.0   # seconds
    ...
    # rate = counts / bin_width  (Hz), will be averaged across trials later
    return counts, t_centers, bin_width


def assign_compartments(probe_coords_mm, config):
    """Assign each electrode channel to SG, G, or IG based on layer z-ranges."""
    z = probe_coords_mm[:, 2]

    # Layer z-ranges from config
    layers = config['layers']
    # Supragranular: L23 (and above)
...
def plot_cleo_psth(trial_dir,
    ...
    # ── Accumulate PSTH counts across trials ──
    all_counts = []
    for d in trials:
    ...
    # Average across trials → rate in Hz
    mean_counts = np.mean(all_counts, axis=0)
    psth = mean_counts / bin_width   # Hz
    psth = smooth_psth(psth, bin_width, smooth_sigma)

    t_ms = t_centers * 1000.0   # for plotting in ms
    depths_mm = probe_coords_mm[:, 2]

    # ── Figure 1: per-channel PSTH ──
    ...
    # ── Figure 2: PSTH by compartment (SG / G / IG) ──
    ...
    # ── Figure 3: LFP by compartment ──
    # Average LFP across trials, then plot per compartment
    all_lfp = []
    for d in trials:
        lfp = d['lfp']             # (n_samples, n_channels)
        lfp_t_ms = d['lfp_t_ms']  # (n_samples,)
        # time-align to stimulus onset
    ...
    mean_lfp = all_lfp.mean(axis=0)   # (n_time, n_channels)

    # time axis from first trial
