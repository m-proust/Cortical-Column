...
from laminar_power_change import load_trials, _time_vector_for_key
...
def detrend_nomatmul(x):
    ...
    xx = x - xm
    denom = (tt * tt).sum()
    slope = (tt * xx).sum() / denom if denom > 0 else 0.0
...
def _band_overlay(ax, x_lo, x_hi, label_bands=True, y_offset=1.0, height=0.04):
    """Draw a colored band-bar above the axis with band names labeled.

    Uses axis-fraction coords in y so it works with any data range / origin.
    """
    ...
    for bname, (b_lo, b_hi) in BANDS.items():
        ...
        if b <= a:
            continue
        rect = plt.Rectangle((a, y_offset), b - a, height,
                             transform=trans, clip_on=False,
                             facecolor=BAND_COLORS[bname], edgecolor='k',
                             linewidth=0.3, alpha=0.95, zorder=10)
        ax.add_patch(rect)
        if label_bands:
            ax.text((a + b) / 2, y_offset + height / 2, bname,
                    transform=trans, ha='center', va='center',
                    fontsize=7, color='#333', zorder=11, clip_on=False)
        ax.axvline(b_lo, color='k', alpha=0.10, lw=0.5, zorder=2)
...
def per_trial_psd(trials, lfp_key, window_ms=WINDOW_MS):
    ...
    for trial in trials:
        ...
        for ch in range(n_channels):
            ...
            f, psd = welch(detrend_nomatmul(seg), fs=fs, window='hann',
...
def run(lesion_root, fig_root, window_ms=WINDOW_MS, n_perm=N_PERM,
    ...
    out_dir = os.path.join(fig_root, '_significance_may21')
    ...
    ax.set_title('Sig-masked log2 FC, averaged across mid-layer (z in [-0.3, +0.7]) channels')
