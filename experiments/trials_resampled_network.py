"""Run trials that redraw the whole network each trial (independent instantiations),
unlike trials.py which fixes the network and reseeds only the noise.

Run from the project root:
    path/to/your/venv/bin/python experiments/trials_resampled_network.py
"""
import os
import sys
...
CONFIG_FILES = [
    ...
    "trials.py",
    "stim_profiles.py",
    "experiments/trials_resampled_network.py",
]


def save_config_snapshot(save_dir, base_dir=None):
    """Copy config + scripts into save_dir/config_snapshot/ for reproducibility."""
    if base_dir is None:
        base_dir = _ROOT
...
if __name__ == "__main__":
    run_multiple_trials(
        ...
        stimuli_ms=3000,
        fs=10000,
        save_dir="saved_trials/resampled_network_03-06",
        verbose=True,
    )
