"""Overlay the kernel and synaptic-current LFP estimates (traces + spectra) for one trial.

Run:
    path/to/your/venv/bin/python analysis/lfp/plot_lfp_compare.py
"""
...
# parameters
TRIALS_DIR = 'results/trials_26_04'
TRIAL = 0
SPACING_SD = 4.0
T0 = 500.0
T1 = 2000.0
...
def _bipolar(x):
    """Diff of consecutive channels: (n_ch-1, T)."""
...
def _plot_psd(t_k, t_c, x_k, x_c, depths, channel_names, t0, t1,
    ...
    # PSD on z-scored signals so amplitudes are comparable across methods
...
def main():
    ...
    trial_path = trials_dir / f'trial_{TRIAL:03d}.npz'
    run_trial(trial_path, trials_dir, T0, T1, SPACING_SD, FMIN, FMAX)
