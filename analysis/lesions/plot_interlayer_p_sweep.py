"""Laminar bipolar power vs an inter-layer connection-probability sweep.
Plot variants (heatmaps, 3D surfaces, single channel) are toggled in main().

Run:
    path/to/your/venv/bin/python analysis/lesions/plot_interlayer_p_sweep.py
"""
...
# parameters
SWEEP_DIR     = "saved_trials/interlayer_p_sweep"
TRANSIENT_MS  = 500
ANALYSIS_MS   = 2000
FREQ_RANGE    = (1, 100)
...
PCT_REF_P     = 0.0          # reference p for % change
NW            = 4            # multitaper time-bandwidth, higher = smoother
SMOOTH_HZ     = 1.5          # PSD smoothing along freq (Hz), 0 disables
...
def _diverging_ticks(vmin, vmax, n_each=3):
    """Colorbar ticks that always label the negative side of a TwoSlopeNorm."""
    def _nice(lo, hi, n):
        if hi <= lo:
            return []
        step = (hi - lo) / n
...
def plot_pct_change_3d_single_channel(p_vals, freqs, psd_stack,
                                      ...
                                      clip_percentile=99):
    """3D surface for one channel: x=freq, y=p, z=% change vs ref_p."""
    n_channels = psd_stack.shape[0]
    bip_depths = ((channel_depths[:-1] + channel_depths[1:]) / 2
                  if len(channel_depths) > n_channels
                  else channel_depths[:n_channels])
    ...
    pct = (psd_stack[ch_idx] - ref_psd) / (ref_psd + 1e-20) * 100.0
    ...
    _mark_3d_peaks(ax, p_up, freqs, pct_up, n=2)
...
def plot_pct_change_3d_surfaces(p_vals, freqs, psd_stack,
    ...
    actual_ref = p_vals[ref_idx]
    ref_psd = psd_stack[:, ref_idx:ref_idx + 1, :]
    pct_all = (psd_stack - ref_psd) / (ref_psd + 1e-20) * 100.0
...
def _band_of(freq):
    """Band name containing freq, or None."""
...
def peak_increase_freq(p_vals, freqs, psd_ch, ref_p=PCT_REF_P):
    """(best_p, peak_freq, peak_pct, band) of the largest power increase vs ref_p."""
    ref_idx = int(np.argmin(np.abs(p_vals - ref_p)))
    ref = psd_ch[ref_idx:ref_idx + 1, :]
    pct = (psd_ch - ref) / (ref + 1e-20) * 100.0
    flat_idx = np.nanargmax(pct)
    p_i, f_i = np.unravel_index(flat_idx, pct.shape)
    peak_freq = float(freqs[f_i])
    return (float(p_vals[p_i]), peak_freq, float(pct[p_i, f_i]),
            _band_of(peak_freq))


def top_increase_peaks(p_grid, freqs, pct, n=2, min_sep_hz=5.0):
    """Top-n (p, freq, pct) maxima of a % change surface, kept min_sep_hz apart."""
    order = np.argsort(pct, axis=None)[::-1]
    peaks = []
    for flat in order:
        if pct.flat[flat] <= 0:
            break
        p_i, f_i = np.unravel_index(flat, pct.shape)
        f = float(freqs[f_i])
        if any(abs(f - pf) < min_sep_hz for _, pf, _ in peaks):
            continue
        peaks.append((float(p_grid[p_i]), f, float(pct[p_i, f_i])))
        if len(peaks) >= n:
            break
    return peaks


def _mark_3d_peaks(ax, p_grid, freqs, pct, n=2):
    """Label the top-n increases on a 3D surface."""
    for pp, ff, vv in top_increase_peaks(p_grid, freqs, pct, n=n):
        ax.scatter([ff], [pp], [vv], color='k', s=60, marker='*',
                   edgecolor='w', linewidth=1.0, depthshade=False, zorder=10)
        ax.text(ff, pp, vv, f'  {ff:.1f} Hz',
                fontsize=8, fontweight='bold', color='k')


def report_peak_increase_per_channel(p_vals, freqs, psd_stack,
                                     channel_labels, ref_p=PCT_REF_P):
    """Print, per channel, the band/frequency that increases most vs ref_p."""
    print(f"\nPeak power increase per channel (vs p={ref_p:.2f}):")
    for ch in range(psd_stack.shape[0]):
        lbl = channel_labels[ch] if ch < len(channel_labels) else f"ch{ch}"
        bp, pf, pct, band = peak_increase_freq(
            p_vals, freqs, psd_stack[ch], ref_p=ref_p)
        print(f"  {lbl:>10s}: +{pct:7.1f}% at {pf:5.1f} Hz "
              f"({band or 'n/a':>5s} band)  @ p={bp:.2f}")


def plot_single_channel(p_vals, freqs, psd_stack,
                        channel_labels, channel_depths,
                        ch_idx=7, ref_p=PCT_REF_P):
    """4-panel figure for one bipolar channel: PSD, % change, 3D surface, band power."""
    n_channels = psd_stack.shape[0]
    bip_depths = ((channel_depths[:-1] + channel_depths[1:]) / 2
                  if len(channel_depths) > n_channels
                  else channel_depths[:n_channels])
    ...
    best_p, peak_freq, peak_pct, band = peak_increase_freq(
        p_vals, freqs, psd_ch, ref_p=ref_p)
    ax1.plot(best_p, peak_freq, marker='*', color='k', markersize=16,
             markeredgecolor='w', markeredgewidth=1.2, zorder=5)
    ax1.annotate(f'{peak_freq:.1f} Hz ({band})\n+{peak_pct:.0f}%',
                 xy=(best_p, peak_freq), xytext=(6, 6),
    ...
    _mark_3d_peaks(ax2, p_up, freqs, pct_up, n=2)
...
def main():
    ...
    report_peak_increase_per_channel(p_vals, freqs, psd_stack,
    ...
    #                     runs[0]['channel_labels'],
    #                     runs[0]['channel_depths'], ch_idx=7)
    plt.show()
