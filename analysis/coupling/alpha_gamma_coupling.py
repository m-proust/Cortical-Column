"""Alpha-gamma cross-frequency coupling: laminar gamma power locked to alpha peaks,
contrasted high- vs low-alpha per cortical compartment.

Run:
    path/to/your/venv/bin/python analysis/coupling/alpha_gamma_coupling.py
"""
import os
import glob
...
def compute_peak_aligned_tfr_raw(
    ...
    """Per-trial peak-aligned raw-power TFR + mean alpha power, for the median split."""
    compartment_labels = classify_bipolar_channels(channel_depths)
    n_channels, n_samples = bipolar_matrix.shape
    window_samp = int(window_ms * fs / 1000)
    warmup_samp = int(warmup_ms * fs / 1000)
...
def aggregate_trials_median_split(trial_dir, n_trials=None, analysis_period='baseline',
    ...
    """Split trials by alpha power, contrast high vs low peak-locked TFR.

    split: 'median' or 'tertile'. contrast: 'logratio' (dB), 'normdiff', or 'diff'.
    """
...
def plot_alpha_gamma_coupling(results, title_suffix='', save_path=None,
    ...
    # narrow 2nd column holds the colorbar so it doesn't shrink the main axes
...
def plot_median_split_contrast(results, title_suffix='', save_path=None):
    """High-minus-low alpha peak-locked TFR contrast per compartment."""
...
def main():
    ...
    args = SimpleNamespace(
        trial_dir='results/trials_06_05-fb',  # folder of trial_*.npz files
        save_dir=None,            # output folder (None -> trial_dir)
        n_trials=None,            # max trials to load (None -> all)
        period='baseline',        # 'baseline', 'stim', 'all', or 'both'
        no_high_alpha=False,      # True -> disable high-alpha segment selection
        alpha_lo=7.0, alpha_hi=14.0,
        gamma_lo=15.0, gamma_hi=200.0, gamma_step=2.0,
        window_ms=300.0,          # half-window around alpha peak (ms)
        percentile=75.0,          # high-alpha threshold percentile
        low_percentile=25.0,      # low-alpha threshold percentile
        split='tertile',          # across-trial split: 'median' or 'tertile'
        contrast='logratio',      # 'logratio', 'normdiff', or 'diff'
        transient_ms=300.0,       # post-stim transient to exclude (ms)
        warmup_ms=500.0,          # initial settling transient to exclude (ms)
        min_ha_ms=150.0,          # minimum high-alpha segment duration (ms)
    )
