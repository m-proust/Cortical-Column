"""
Run trials with cleo MUA and LFP recording, then save per-channel
spike data (mua.t, mua.i) and LFP (rwslfp.lfp) for offline PSTH analysis.
"""
import os
import shutil
import numpy as np
...
def run_single_trial(
    ...
    # ── Set up cleo simulator with MUA + LFP on the probe ──
    ...
    # Inject probe into E populations (they carry the PSCs for LFP)
    ...
    # ── Stimulus phase (currently no extra stimulus, same as trials.py) ──
    if verbose:
    ...
    # ── Extract cleo MUA data ──
    mua_t_ms = np.array(mua.t / b2.ms)
    mua_i = np.array(mua.i)

    # ── Extract cleo LFP data ──
    lfp = np.array(rwslfp.lfp)         # shape: (n_samples, n_channels)
    lfp_t_ms = np.array(rwslfp.t / b2.ms)

    # ── Also save spike monitors per layer (useful for other analyses) ──
