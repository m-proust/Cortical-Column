"""
Same as trials.py but also computes and saves the Mazzoni per-layer LFP proxy.
"""
import os
import shutil
import numpy as np
...
def run_single_trial(
    ...
    mazzoni_lfp, mazzoni_time = calculate_lfp_mazzoni_perlayer(
        ...
        CONFIG['layers'],
        fs=fs,
        dt_ms=1.0,  # StateMonitor records at dt=1*ms (see layer.py:228)
    )
    ...
    # Mazzoni per-layer: store layer names and a matrix (layers x time)
    mazzoni_layer_names = list(mazzoni_lfp.keys())
    mazzoni_matrix = np.vstack([mazzoni_lfp[ln] for ln in mazzoni_layer_names])
    data["mazzoni_layer_names"] = np.array(mazzoni_layer_names, dtype=object)
    data["mazzoni_lfp_matrix"] = mazzoni_matrix
    data["mazzoni_time_ms"] = np.array(mazzoni_time)
...
def run_multiple_trials(
    ...
    for trial_id in range(34,n_trials):
        ...
        save_dict = {
            ...
            "mazzoni_layer_names": data["mazzoni_layer_names"],
            "mazzoni_lfp_matrix": data["mazzoni_lfp_matrix"],
            "mazzoni_time_ms": data["mazzoni_time_ms"],
        }
