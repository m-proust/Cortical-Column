...
for i, layer in enumerate(layers):
    ...
    ax.set_ylabel('Spike-LFP phase locking', fontsize=12)
    ax.set_xlim(FREQ_RANGE)
    ax.set_ylim(bottom=0)
    ax.legend(fontsize=11)
    ax.set_title(f'{layer}', fontsize=13, fontstyle='italic')
    if i == n_layers - 1:
        ax.set_xlabel('Frequency (Hz)', fontsize=12)

plt.tight_layout()
plt.savefig('ppc_spectrum.png', dpi=150, bbox_inches='tight')
plt.show()
...
for layer in layers:
    ...
    ax_ph.set_xlabel('spike gamma-phase', fontsize=12)
